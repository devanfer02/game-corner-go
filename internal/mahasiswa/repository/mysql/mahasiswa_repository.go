...
func NewMysqlMahasiswaRepository(conn *sql.DB) domain.MahasiswaRepostory {
	return &mysqlMahasiswaRepostiory{conn}
}

func (sql *mysqlMahasiswaRepostiory) fetch(ctx context.Context, query string, args ...interface{}) ([]domain.Mahasiswa, error) {
...
func (sql *mysqlMahasiswaRepostiory) Register(ctx context.Context, mhs *domain.Mahasiswa) error {
	query := `INSERT INTO mahasiswa (nim, email, password, nama, jurusan, updated_at, created_at) 
				VALUES (?, ?, ?, ?, ?, ?, ?)`
	...
	_, err = stmt.ExecContext(ctx, mhs.NIM, mhs.Email, mhs.Password, mhs.Nama, mhs.Jurusan, mhs.UpdatedAt, mhs.CreatedAt)
	if err != nil {
		return err 
	}
